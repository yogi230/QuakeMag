...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String URGS_REQUEST = " https://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&eventtype=earthquake&orderby=time&minmag=4&limit=30";
    private static String realUrgsJson = "";
    ...
    public void updateUI() {
        ...
        if (realUrgsJson == "") {
            Log.v(LOG_TAG, "unable to fetch json string response");
            realUrgsJson = SAMPLE_JSON_RESPONSE;
        }
//        Log.v(LOG_TAG,"this is my realjson= "+realUrgsJson);
        ArrayList<Item> arrayList = querylist.extractItemList(realUrgsJson);
        final ItemAdapter itemAdapter = new ItemAdapter(this, arrayList);
        listView.setAdapter(itemAdapter);
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                Item items = itemAdapter.getItem(position);
                Uri earth = Uri.parse(items.getUrl());
                Intent website = new Intent(Intent.ACTION_VIEW, earth);
    ...
    private class TaskAsync extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
            ...
            String s = Netw.extractdata(strings[0]);
//            Log.v(LOG_TAG,"this is my doinbackground jsonresponse = "+ s);
            return s;
...
//            Log.v(LOG_TAG,"on post execute my json resposnse is "+ string);
            realUrgsJson = string;
            updateUI();
        }
    }
